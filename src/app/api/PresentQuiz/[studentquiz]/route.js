...
export async function GET(request) {
  ...
  //   here is calculation of present time according to indian clock
    ...
    const query = `SELECT * FROM quiztable WHERE StartTime <= ? AND EndTime >= ? AND JSON_CONTAINS(Domains, JSON_ARRAY(?))`;
    const [result] = await db.promise().execute(query, [currentTime, currentTime,domain]);
