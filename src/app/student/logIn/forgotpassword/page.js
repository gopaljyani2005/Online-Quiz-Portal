...
export default function forgotpassword(){
    ...
    async function sendotpfunct(e){
        ...
        if(response1.ok){
            const studentdetail = await response1.json();
            const Username = studentdetail.Username;
    ...
    async function otpverification(e){
        ...
        if(otp===sendotp){
            const encodedUsername = encodeURIComponent(Username);
            router.push(`/student/logIn/forgotpassword/resetpassword?Username=${encodedUsername}`);
        }
        else{
